class Renderer{
    constructor() {
        ...
        this._renderMethod = 'svg';
        this._curNodeId = null;
        this._withLabel = 1;
        this._force_link = 100;
        this._force_manybody = 1500;
...
function showAllNodesInfo(sortedNodes){
    ...
    nodesList.html('');
    $('#all_nodes_cnt').text(sortedNodes['nodes'].length + 'vertices');
    var data;
    for(var n of sortedNodes['nodes']){
        data = "{'id': "+n.id+", 'type': "+n.type+", 'prob': "+n.prob+", 'name': '"+n.name+"'}";
...
function showAllLinksInfo(sortedLinks){
    let nodesList = $('#show_all_edges');
    nodesList.html('');
    $('#all_edges_cnt').text(sortedLinks.length + ' edges');
    var data;
    for(var n of sortedLinks){
    console.log(n)
        data = "{'id': "+n.id+", 'type': "+n.type+", 'prob': "+n.prob+", 'name': '"+n.name+"'}";
        let query_classname = n['query'] == '1' ? 'Q' : '';
        nodesList.append('<div class="menu_body_line" nid="804" onclick="" onmouseover="" onmouseleave="">' +
                        '    <div style="display: flex;">' +
                        '        <span class="node" style="background: '+GRAPH['color'][GRAPH['nodes'][n.source].type]+'; display:inline-block;"></span>' +
                        '        <span class="node" style="background: '+GRAPH['color'][GRAPH['nodes'][n.target].type]+'; margin-left: 3px; display:inline-block;"></span>' +
                        '    </div>' +
                        '    <span class="list_item">('+n.source+','+n.target+')</span>' +
                        '    <span class="list_item">'+n.score+'</span>' +
                        '</div>');
    }
}
...
function showNodeStatistics() {
    ...
    com_search_acc.text(((same / (same + diff)) * 100).toFixed(5) + '%');
    com_node.text(same + diff);
    com_same.text(same);
    com_diff.text(diff);
}

function initGraphData(query_node, graph) {
    GRAPH['nodes'] = graph["NODES"];
    GRAPH['links'] = graph["LINKS"];
    GRAPH['ig'] = graph["ig"];
    GRAPH['queryId'] = query_node;
...
function renderGraph(snapshot){
    snapshot = parseInt(snapshot)
    let query_node = G_JSON['QueryNode'];
    let snap_num = G_JSON['snap_num'];

    if(snapshot == 0){
        let static_graph = G_JSON['static'];
        initGraphData(query_node, static_graph);
        renderGraphByMethod();
    }else if(snapshot > 0 && snapshot <= snap_num){
        let dynamic_graph = G_JSON['dynamic'][snapshot - 1];
        initGraphData(query_node, dynamic_graph);
        renderGraphByMethod();
    }
}

function renderGraphNavigator(){
    let graph_navigator = $("#graph_navigator");
    let snap_num = G_JSON['snap_num'];
    
    graph_navigator.html('<span class="tab selected_tab" id="g_tab_snap_0" onclick="onChangeGraphNavigator(0)">Static</span>');
    for(i = 1; i <= snap_num; i++){
        graph_navigator.append('<span class="tab" id="g_tab_snap_' + i + '" onclick="onChangeGraphNavigator(' + i + ')">Snapshot ' + i + '</span>');
    }

    CURRENT_G_NAV_TAB = 0;
}

function onChangeGraphNavigator(snapshot){
    snapshot = parseInt(snapshot)
    let snap_num = G_JSON['snap_num'];

    if(snapshot < 0 || snapshot > snap_num){
        return False;
    }
    
    $('#g_tab_snap_' + CURRENT_G_NAV_TAB).removeClass('selected_tab');
    $('#g_tab_snap_' + snapshot).addClass('selected_tab');
    CURRENT_G_NAV_TAB = snapshot;
...
function output_logs_line(type, text){
    var logs_area = $("#float_logs_text");
    ...
    logs_area.append('<div class="logs_line">\n' +
                    '<span class="logs_label">' + type + '</span>\n' +
                    '<span class="logs_text">' + text + '</span>\n' +
                    '</div>');
}

function search_community(query_data){
    $('#float_logs').fadeIn();
    var text_area = $("#float_logs_text");
    output_logs_line("out", JSON.stringify(query_data));

    $.ajax({
        url: "CommunitySearch/search",
        type: "POST",
        data: JSON.stringify(query_data),
        dataType: "json",
        success: function (data) {
            console.log(data);
            output_logs_line("in", JSON.stringify(data));
            if(data['isOK']){
                G_JSON = data['G_JSON'];
                $('#float_logs').fadeOut();
...
$("#pause_graph").click(function () {
    showTip('Pausing the movement');
    SIMULATION.alphaTarget(0);
});

$("#reset_graph").click(function () {
    showTip('Reset image position');
    if(RENDERER.renderMethod == 'svg'){
        resetDrag();
    }else if(RENDERER.renderMethod == 'canvas'){
        resetTransform();
...
$("#search_set").click(function () {
    ...
    if(GRAPH['ig'] == null || GRAPH['ig'] == undefined || GRAPH['ig'].length == 0){
        search_res.append('ig is not defined');
        $("#search_cnt").text('');
        return false;
    }
    ...
    $("#search_cnt").text('Total: '+cnt);
});

$("#upgrade_graph").click(function () {
    showTip('Applying parameters');
...
$("#search_community").click(function () {
    showTip('Searching communities');
    var query_data = {
        param_q: $.trim($("#param_q").text()),
        param_w: $.trim($("#param_w").val()),
        param_s: $.trim($("#param_s").val())
    };

    search_community(query_data);
});

$("#new_search").click(function () {
    $('#new_query_dialog').fadeIn(200);
});
